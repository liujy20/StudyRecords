...
export default {
  ...
  filters: {
    filterProducts(goods, data) {
      console.log(goods);
      let { spellPeople, searchStatus, minPrice, maxPrice } = data;
      let temp = [];
      switch (+spellPeople) {
        case 1:
          temp = goods.filter((item) => {
            return item.spellPeople == 1;
          });
          console.log("temp1", temp);
          break;
        case 2:
          temp = goods.filter((item) => {
            return item.spellPeople == 2;
          });
      ...
      if (searchStatus !== "") {
        ...
        console.log("statusList", temp);
      }
      return temp.filter((item) => {
        return item.product.price >= minPrice && item.product.price <= maxPrice;
      });
    },
  },
};
